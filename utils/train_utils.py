...
def add_softmax_cross_entropy_loss_for_each_scale(scales_to_logits,
  ...
  for scale, logits in six.iteritems(scales_to_logits):
    ...
    label1_weight = 1 # qtz
    label2_weight = 10 # q
    label0_weight = 10 # others
    ignore_weight = 0 # ignore

    # not_ignore_mask = tf.to_float(tf.not_equal(scaled_labels,
    #                                          ignore_label)) * loss_weight
    not_ignore_mask = tf.to_float(tf.equal(scaled_labels,1))*label1_weight + tf.to_float(tf.equal(scaled_labels,2))*label2_weight + tf.to_float(tf.equal(scaled_labels,0))*label0_weight  + tf.to_float(tf.equal(scaled_labels, ignore_label))*ignore_weight 
